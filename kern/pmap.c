...
mem_init_mp(void)
{
	...
	boot_map_region(kern_pgdir,kstack_start,KSTKSIZE,PADDR(percpu_kstacks[i]),PTE_W);//-i*(KSTKSIZE+KSTKGAP)
...
page_init(void)
{
	...
	for (i = 1; i < MPENTRY_PADDR/PGSIZE; i++) {		//vyhodena 0.page npages_basemem
	...
	int mem=(int)ROUNDUP(((char*)envs) + (sizeof(struct Env) * NENV) - 0xf0000000, PGSIZE)/PGSIZE;//PGNUM(PADDR(boot_alloc(0)));
	...
	// 2. Add rest of base memory to page_free_list.
	...
	// 3. IO hole should not be allocated, so not in page_free_list.

	// 4. Pages after boot_alloc()'s so-far allocation are all free.
...
if(!create)
return 0;
struct PageInfo* pgtbl=page_alloc(1);
if(pgtbl==NULL)
return NULL;
else{
pgtbl->pp_ref++;
pte=(pte_t*)(page2kva(pgtbl));
*pde = page2pa(pgtbl) |PTE_W|PTE_P|PTE_U ;
}
}
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	size-=pa;
	if(base+size>=MMIOLIM)panic("mmio_map_region:out of mmio memory");
	...
	pa = ROUNDDOWN(pa, PGSIZE);
	size -= pa;
	if (base+size >= MMIOLIM) panic("mmio_map_region:not enough memory");
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uint32_t i;
	for(i=(uint32_t)addr;i<end;i+=PGSIZE){
	...
	//cprintf("user_mem_check_success va: %x, len: %x\n",va,len);
	return 0;
}
