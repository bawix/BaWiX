...
trap_init(void)
{
	...
	extern void TH_BRKPT(); 	SETGATE(idt[T_BRKPT], 0, GD_KT, TH_BRKPT, 3); // 3
	...
	extern void TH_SYSCALL(); 	SETGATE(idt[T_SYSCALL], 0, GD_KT, TH_SYSCALL, 3); //pozor ak  bude padat :D  0 1  // 3
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if(curenv->env_pgfault_upcall &&
	   (tf->tf_esp<USTACKTOP || tf->tf_esp>=UXSTACKTOP-PGSIZE)){
		...
		user_mem_assert(curenv,(void*)uxtop,UXSTACKTOP-uxtop,PTE_P|PTE_U);
